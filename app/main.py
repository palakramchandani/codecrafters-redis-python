            ...
            if cmd == 'MULTI' and len(command_parts) == 1:
                ...
                if in_multi:
                    if len(queued_commands) == 0:
                        # Empty transaction - send empty array
                        connection.sendall(b'*0\r\n')
                    else:
                        # Future: execute queued commands here
                        pass
                    in_multi = False
                    queued_commands.clear()
                else:
                ...
                if in_multi:
                    # Queue commands during MULTI, do not execute
                    queued_commands.append(command_parts)
                    connection.sendall(b'+QUEUED\r\n')
                else:
                    # NOT in multi mode - execute commands immediately
                    ...
                    if cmd == 'SET' and len(command_parts) >= 3:
                        ...
                        if any(len(entries) > 0 for _, entries in resp_data) or block is None:
                            ...
                            # Blocking requested; implementation skipped here
                            pass
